...
const YourThoughts = ({ userId, username, location }) => {
    ...
    const handleLikes = async (thought, net) => {
        if (net > 0) {
            try {
                const thoughtsResponse = await axios.patch(`http://localhost:4000/endpoints/thoughts/${thought._id}/like/addLike`, {
                    currentLikeCount: thought.likeCount,
                })
                const likesResponse = await axios.post(`http://localhost:4000/endpoints/likes/likeOnThought`, {
                    userId: userId,
                    thoughtId: thought._id,
                })
                if (thoughtsResponse.status == 200 && likesResponse.status == 200) {
                    fetchData();
                    console.log("status 200")
                } else {
                    console.error('Failed to like thought. Status:', thoughtsResponse.status);
                }
            } catch (error) {
                console.error('Error:', error.message);
            }
        } else {
            console.log("supposed to dislike")
        }
    }
                ...
                {activeThoughts.map(thought => (
                    <View key={thought._id} style={styles.thoughtBubble}>
                        ...
                        <View style={styles.metadataContainer}>
                            <TouchableOpacity style={styles.likeCountContainer} onPress={() => handleLikes(thought, 1)}>
